...
- The seed also gives every contestant the same random budget. Gold ranges rise by 20 at both ends per floor: 80–250, 100–270, 120–290, 140–310, then 160–330. Every floor grants 0–2 Tears of the Runner.
...
- One Tear builds a one-cell **Tower of Lament**. When the runner enters any of its eight surrounding cells, the tower halves its speed for five seconds, then needs five seconds before it can trigger again. Slows refresh but do not stack.
- A neutral **speed tower** increases the runner's movement speed by 50% for five seconds whenever the runner enters any of its eight surrounding cells. It has no cooldown, so a later adjacent-square entry refreshes the effect. Speed and slow effects multiply, so simultaneous 1.5x and 0.5x effects produce 0.75x movement.
- A linked **portal pair** has two protected ends in random open cells. Portal ends always have a full playable-tile buffer from the board edge and black silhouette cells. Entering either active end instantly moves the runner to its partner; that pair then deactivates for the rest of the run. Multiple pairs operate independently. The runner never seeks a portal as a shortcut—it follows the ordinary shortest route, and reroutes normally from the exit only if that route happens to enter one.
- A **Trap Door** is a protected, traversable floor object that always has a full playable-tile buffer from the board edge and black silhouette cells. When an ordinary shortest route crosses one, it launches the runner up to three squares in the current direction in the time of one normal square, jumping over intervening blockers and stopping at the last valid square before a boundary, then deactivates. The runner never detours to use one.
